...
try {
    mysqli_connect($_ENV['apiHost'], $_ENV['apiUser'], $_ENV['apiPassword']);
    mysqli_select_db($_ENV['apiDatabase']);

    $apiTable = $_ENV['apitable'];
    $apiC2    = $_ENV['column2'];
    $apiC3    = $_ENV['column3'];

    if ($filter != "") {
        $query = "
          SELECT DATE_FORMAT(STR_TO_DATE($apiC2,'%a, %d %b %Y %T'), '%d %m %Y') day, count(id) visitors
            FROM $apiTable
            WHERE $apiC3 LIKE '%$filter%'
            GROUP BY DATE_FORMAT(STR_TO_DATE($apiC2,'%a, %d %b %Y %T'), '%d %b %Y')
            ORDER BY DATE_FORMAT(STR_TO_DATE($apiC2,'%a, %d %b %Y %T'), '%Y') desc,
              DATE_FORMAT(STR_TO_DATE($apiC2,'%a, %d %b %Y %T'), '%m') desc,
              DATE_FORMAT(STR_TO_DATE($apiC2,'%a, %d %b %Y %T'), '%d') desc LIMIT 1000";
    } else {
        $query = "
          SELECT DATE_FORMAT(STR_TO_DATE($apiC2,'%a, %d %b %Y %T'), '%d %b %Y') day, count(id) visitors
            FROM $apiTable
            GROUP BY DATE_FORMAT(STR_TO_DATE($apiC2,'%a, %d %b %Y %T'), '%d %b %Y')
            ORDER BY DATE_FORMAT(STR_TO_DATE($apiC2,'%a, %d %b %Y %T'), '%Y') desc,
              DATE_FORMAT(STR_TO_DATE($apiC2,'%a, %d %b %Y %T'), '%m') desc,
