<?php
$i18n = array(
    "from" => "Van",
    "to" => "Naar",
    "date" => "Datum",
    "time" => "Tijd",
    "arrival" => "Aankomst",
    "depart" => "Vertrek",
    "search" => "Zoek",
    "switch" => "Wissel om",
    "save" => "Opslaan",
    "settings" => "Configuratie",
    "savedinacookie" => "Opgeslagen in een koekje! Jamjam.",
    "language" => "Taal",
    "about" => "Over ons",
    "back" => "Terug",
    "map" => "Kaart",
    "duration" => "Duur",
    "delay" => "Vertraging",
    "platform" => "Perron",
    "transfers" => "Overstappen",
    "transportation" => "Toestellen",
    "error" => "Fout",
    "errormessage" => "We could not provide you with an answer to your question. Most likely you requested too old information, or information that we can't provide yet. If you think this error is a bug in iRail, please let us know on project.iRail.be",
    "strike"=> "Op deze dag heeft de NMBS stakingen aangekondigd. Vertrouw niet op deze informatie!"
    , "news" => "Nieuws"
    , "feedback" => "Feedback"
    ,"options" =>"Opties"
    , "setdatetime" => "Kies de tijd en datum"

    );

?>
