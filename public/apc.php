...
defaults('ADMIN_PASSWORD', getenv('APC_UI_PASSWORD'));      // Admin Password - CHANGE THIS TO ENABLE!!!
...
if (isset($MYREQUEST['IMG'])) {
    ...
    switch ($MYREQUEST['IMG']) {
        ...
        case 2:
            ...
            fill_box($image, 30, $size, 50, $s ? (-$a * ($size - 21) / $s) : 0, $col_black, $col_green,
                sprintf('%.1f%%', $s ? $cache['num_hits'] * 100 / $s : 0));
            fill_box($image, 130, $size, 50, $s ? -max(4, ($s - $a) * ($size - 21) / $s) : 0, $col_black, $col_red,
                sprintf('%.1f%%', $s ? $cache['num_misses'] * 100 / $s : 0));
            break;
...
function put_login_link($s = 'Login')
{
    ...
    if (!USE_AUTHENTICATION) {
        ...
        print <<<EOB
            <a href="#" onClick="alert('You need to set a password at the top of apc.php before this will work!');return false";>$s</a>
EOB;
...
if ($AUTHENTICATED) {
    echo <<<EOB
        <li><a class="aright" href="$MY_SELF&CC=1&OB={$MYREQUEST['OB']}" onClick="return confirm('Are you sure?');">Clear Cache</a></li>
EOB;
}
echo <<<EOB
    </ol>
...
EOB;
        ...
        '<td class=td-1><span class="green box">&nbsp;</span>Hits: ', $cache['num_hits'] . @sprintf(' (%.1f%%)',
                $cache['num_hits'] * 100 / $num_hits_and_misses), "</td>\n",
        '</tr>',
        '<tr>',
        '<td class=td-0><span class="red box">&nbsp;</span>Used: ', bsize($mem_used) . sprintf(' (%.1f%%)', $mem_used * 100 / $mem_size), "</td>\n",
        '<td class=td-1><span class="red box">&nbsp;</span>Misses: ', $cache['num_misses'] . @sprintf(' (%.1f%%)',
                $cache['num_misses'] * 100 / $num_hits_and_misses), "</td>\n";
        echo <<< EOB
        </tr>
        </tbody></table>
        ...
        if ($list) {
            ...
            foreach ($list as $k => $entry) {
                if (empty($MYREQUEST['SEARCH_REGEX']) || preg_match($MYREQUEST['SEARCH_REGEX'], $entry[$fieldname]) != 0) {
                    ...
                    if ($sh == $MYREQUEST['SH']) {
                        echo '<tr>';
                        echo '<td colspan="7"><a class="entry-info" href="' . $href . '"><pre>' . htmlentities(print_r(apcu_fetch($entry['info']),
                                1)) . '</pre></a></td>';
                        echo '</tr>';
                    }
                    $i++;
