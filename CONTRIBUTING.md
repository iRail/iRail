...
The [issue tracker](https://github.com/irail/irail/issues) is preffered channel for bug reports, features requests and
...
A bug is a *demonstrable problem* that is caused by the code in the code in the repository. Good bug reports are
extremely helpful,
so thanks!
...
3. **Check if the issue has been fixed** - try to reproduce the issue using the latest `master` branch in the
   repository.
4. **Isolate the issue**
...
What browser(s) and OS experience the problem? Do other browsers show the bug differently? What would you expect to be
the outcome?
