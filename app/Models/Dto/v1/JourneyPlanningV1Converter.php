...
class JourneyPlanningV1Converter extends V1Converter
{
    ...
    private static function convertJourneyPlan(Journey $journey)
    {
        ...
        $result->vias = self::convertVias($journey->getLegs());
        $result->duration = $journey->getDurationSeconds();
        $result->remarks = new StdClass(); // TODO: implement
        $result->alerts = new StdClass(); // TODO: implement
        return $result;
    }

    private static function convertDeparture(\Irail\Models\DepartureOrArrival $departure, JourneyLeg $departureLeg): StdClass
    {
        $result = new StdClass();
    ...
    private static function convertArrival(\Irail\Models\DepartureOrArrival $arrival, JourneyLeg $arrivalLeg): StdClass
    {
        $result = new StdClass();
        $result->delay = $arrival->getDelay();
        $result->station = self::convertStation($arrival->getStation());
        $result->time = $arrival->getScheduledDateTime();
        ...
        $result->stop = array_map(fn($stop) => self::convertIntermediateStop($stop), $arrivalLeg->getIntermediateStops());
        ...
        $result->walking = $arrival->getVehicle()->getName() == "WALK" ? '1' : '0';
        $result->alerts = [];
        return $result;
    }
