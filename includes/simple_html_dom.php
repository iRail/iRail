 ...
 * and after the remove_noice calls so it will not reflect the REAL position of the tag in the source,
 *
...
public function file_get_html($url, $use_include_path = false, $context = null, $offset = -1, $maxLen = -1, $lowercase = true, $forceTagsClosed = true, $target_charset = DEFAULT_TARGET_CHARSET, $stripRN = true, $defaultBRText = DEFAULT_BR_TEXT)
{
    ...
    // For sourceforge users: uncomment the next line and comment the retreive_url_contents line
    // 2 lines down if it is not already done.
 ...
 *
 * Paperg - in the find routine: allow us to specify that we want case insensitive testing
 * of the value of the selector.
 *
 * Paperg - change $size from protected to public so we can easily access it
 *
 * Paperg - added ForceTagsClosed in the constructor which tells us whether we trust the html or not.
 * Default is to NOT trust it.
 *
 * @package PlaceLocalInclude
 */
class simple_html_dom
{
    ...
    /**
     * Clean up memory due to php5 circular memory leak...
     */
    function clear()
    {
        foreach ($this->nodes as $n) {
    ...
    /**
     * PAPERG - dkchou
     * added this to try to identify the character set of the page we have just parsed so we know
     * better how to spit it out later.
     *
     * NOTE:
     * IF you provide a routine called get_last_retrieve_url_contents_content_type
     * which returns the CURLINFO_CONTENT_TYPE fromt he last curl_exec
    ...
    protected function parse_charset()
    {
        ...
        if (empty($charset)) {
            ...
            // then we need to just wrongheadedly assume it's UTF-8 so that we can move on.
    ...
    protected function parse_attr($node, $name, &$space)
    {
        ...
        // If the attribute is already defined inside a tag,
        // only pay atetntion to the first one as opposed to the last one.
        ...
        // PaperG: If this is a "class" selector,
        // lets get rid of the preceeding and trailing space since some people leave it in the multi class case.
        if ($name == "class") {
